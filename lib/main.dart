...
void callbackDispatcher() {
  Workmanager().executeTask((task, inputData) async {
    ...
    if (task == 'autoResumeDownloads') {
       // Only run if the feature is enabled in settings (default true or false)
      ...
      builder: (ColorScheme? lightDynamic, ColorScheme? darkDynamic) {
        ...
        if (appState.trueAmoledDark) {
          darkScheme = darkScheme.copyWith(
            ...
            surfaceContainerLow: const Color(0xFF0D1117), // Slightly lighter for containers
        ...
        return MaterialApp(
          ...
          darkTheme: ThemeData.dark(useMaterial3: true).copyWith(
            colorScheme: darkScheme,
            scaffoldBackgroundColor: appState.trueAmoledDark ? Colors.black : null,
...
class _BiometricLockWrapperState extends State<BiometricLockWrapper> with WidgetsBindingObserver {
  ...
  Future<void> _checkAuth() async {
    ...
    try {
      ...
      if (mounted) setState(() => _isAuthenticated = true); // Fallback to let user in if error
    }
  }
      ...
      body: Center(
        child: Column(
          ...
          children: [
            ...
            const Text('App Locked', style: TextStyle(color: Colors.white, fontSize: 24)),
  ...
  Widget build(BuildContext context) {
    return Consumer<DownloadProvider>(
      builder: (context, provider, child) {
        final activeItems = provider.queue.where((i) => i.status == DownloadStatus.downloading).toList();
        ...
        final avgProgress = totalActive > 0 ? (totalProgress / totalActive) / 100 : 0.0;
