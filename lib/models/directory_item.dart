enum DirectoryItemType { directory, video, audio, image, archive, document, other }
...
class DirectoryItem {
  ...
  static String? _formatSize(String? s) {
    ...
    try {
      if (s.endsWith('K') || s.endsWith('KB')) {
        ...
        b = double.parse(s.replaceAll(RegExp(r'[KMBG]'), '').trim()) * 1024 * 1024;
      } else if (s.endsWith('G') || s.endsWith('GB')) {
        b = double.parse(s.replaceAll(RegExp(r'[KMBG]'), '').trim()) * 1024 * 1024 * 1024;
      } else {
  ...
  static DirectoryItemType typeFromExtension(String name) {
    ...
    const videoExt = ['mp4', 'mkv', 'avi', 'mov', 'wmv', 'flv', 'm4v', 'webm', 'ts', 'm2ts'];
    ...
    const docExt = ['pdf', 'doc', 'docx', 'xls', 'xlsx', 'txt', 'epub', 'mobi', 'srt', 'nfo'];
