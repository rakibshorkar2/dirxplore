          ...
          PopupMenuButton<String>(
              ...
              if (value == 'export') {
                ...
                if (context.mounted) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(content: Text(success ? 'Queue imported successfully!' : 'Import cancelled or failed.')),
                  );
                }
              }
            },
            itemBuilder: (context) => [
              const PopupMenuItem(
                value: 'export',
                child: Row(
                  children: [Icon(Icons.upload_file, size: 20), SizedBox(width: 8), Text('Export Queue')],
                ),
              ),
              const PopupMenuItem(
                value: 'import',
                child: Row(
                  children: [Icon(Icons.download_rounded, size: 20), SizedBox(width: 8), Text('Import Queue')],
                ),
              ),
            ],
          ),
          ...
          child: Column(
            ...
            children: [
              Row(
                ...
                children: [
                  const Text('Device Storage', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13)),
                  Text(
              ...
              ClipRRect(
                ...
                child: LinearProgressIndicator(
                  value: progress,
                  minHeight: 6,
                  color: progress > 0.9 ? Colors.red : Theme.of(context).colorScheme.primary,
  ...
  void _confirmClearDone(BuildContext context, DownloadProvider dlProvider) {
     showDialog(
       ...
       builder: (ctx) => AlertDialog(
         ...
         content: const Text('This will remove completed and failed tasks from the list. Files will remain on disk.'),
         actions: [
           ...
           TextButton(
             ...
             child: const Text('Clear List', style: TextStyle(color: Colors.red))
           ),
         ]
       )
     );
      ...
      child: ExpansionTile(
        ...
        subtitle: Column(
          ...
          children: [
            ...
            const SizedBox(height: 4),
            Text('${items.where((i) => i.status == DownloadStatus.done).length} of ${items.length} files done', style: const TextStyle(fontSize: 12)),
          ],
        ),
                ...
                children: [
                  if (item.status == DownloadStatus.done)
                    TextButton.icon(
                      ...
                      label: const Text('Verify Hash', style: TextStyle(fontSize: 12)),
                      style: TextButton.styleFrom(
                        padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 0),
                        minimumSize: Size.zero,
                        tapTargetSize: MaterialTapTargetSize.shrinkWrap,
                      ),
                      onPressed: () => _showVerifyHashDialog(context, dlProvider, item),
                    ),
                  if (item.status == DownloadStatus.downloading || item.status == DownloadStatus.queued)
                    IconButton(
            ...
            return AlertDialog(
              ...
              content: Column(
                ...
                children: [
                  ...
                  CheckboxListTile(
                    title: const Text('Delete file from storage as well', style: TextStyle(fontSize: 13)),
                    value: deleteFile,
          ...
          builder: (context, setState) {
            return AlertDialog(
              ...
              content: Column(
                mainAxisSize: MainAxisSize.min,
                children: [
                  Text('Check MD5 or SHA256 for: ${item.fileName}', style: const TextStyle(fontSize: 12)),
                  const SizedBox(height: 10),
                  TextField(
                    controller: ctrl,
